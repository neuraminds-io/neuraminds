...
pub fn encrypt_json(master_key: &str, key_id: &str, payload: &serde_json::Value) -> Result<String, ApiError> {
...
pub fn decrypt_json(master_key: &str, key_id: &str, encrypted: &str) -> Result<serde_json::Value, ApiError> {
    if master_key.trim().is_empty() {
        return Err(ApiError::internal(
            "EXTERNAL_CREDENTIALS_MASTER_KEY is not configured",
        ));
