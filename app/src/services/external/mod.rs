...
fn http_client() -> Result<Client, ApiError> {
    Client::builder()
        ...
        .build()
        .map_err(|err| ApiError::internal(&format!("failed to build external http client: {}", err)))
}
...
pub async fn fetch_markets(
    ...
    if matches!(source, ExternalMarketSource::All | ExternalMarketSource::Limitless)
        && config.limitless_enabled
    {
        ...
        .await?;
        markets.extend(fetched.into_iter().filter(|entry| include_market(tradable_filter, entry)));
    }

    if matches!(source, ExternalMarketSource::All | ExternalMarketSource::Polymarket)
    ...
    markets.sort_by(|a, b| b.close_time.cmp(&a.close_time).then_with(|| a.id.cmp(&b.id)));
...
pub async fn fetch_orderbook(
    ...
    let value = match market_id.provider {
        ExternalProvider::Limitless => with_retries(|| {
        ...
        .await?,
        ExternalProvider::Polymarket => with_retries(|| {
