...
fn parse_limitless_market(entry: &Value) -> Option<ExternalMarketSnapshot> {
    ...
    let outcome = match entry.get("winningOutcomeIndex").and_then(|value| value.as_u64()) {
    ...
    Some(ExternalMarketSnapshot {
        ...
        question: if title.is_empty() { slug.clone() } else { title },
        description,
        category,
...
pub async fn fetch_trades(
    ...
    if let Some(events) = payload.get("events").and_then(|value| value.as_array()) {
        for (index, event) in events.iter().enumerate() {
            let side = event.get("side").and_then(|value| value.as_i64()).unwrap_or(0);
