...
async fn submit_to_provider(
    ...
    match provider {
        ExternalProvider::Limitless => {
            let api_key = api_key_from_payload(&credential.payload, &["apiKey", "api_key"]).ok_or_else(|| {
                ApiError::bad_request(
                    "INVALID_CREDENTIALS",
        ...
        ExternalProvider::Polymarket => {
            ...
            let api_secret = api_key_from_payload(&credential.payload, &["apiSecret", "api_secret"])
                .ok_or_else(|| {
                    ApiError::bad_request(
                        "INVALID_CREDENTIALS",
            ...
            let api_passphrase = api_key_from_payload(
                &credential.payload,
...
fn build_external_order_response(row: sqlx::postgres::PgRow) -> Result<ExternalOrderResponse, ApiError> {
...
fn parse_external_agent(row: sqlx::postgres::PgRow) -> Result<ExternalAgentResponse, ApiError> {
    ...
    Ok(ExternalAgentResponse {
        ...
        owner: row
            .try_get("owner")
            .map_err(|err| ApiError::internal(&err.to_string()))?,
        name: row.try_get("name").map_err(|err| ApiError::internal(&err.to_string()))?,
        provider: row
            .try_get("provider")
            .map_err(|err| ApiError::internal(&err.to_string()))?,
        market_id: row
        ...
        outcome: row
            .try_get("outcome")
            .map_err(|err| ApiError::internal(&err.to_string()))?,
        side: row.try_get("side").map_err(|err| ApiError::internal(&err.to_string()))?,
        price: row.try_get("price").map_err(|err| ApiError::internal(&err.to_string()))?,
        quantity: row
            .try_get("quantity")
            .map_err(|err| ApiError::internal(&err.to_string()))?,
        cadence_seconds: row
...
pub async fn submit_external_order(
    ...
    let provider_response = submit_to_provider(&state, provider, &credential, &body.signed_order).await;
    let now = Utc::now();
    ...
    sqlx::query(
        "UPDATE external_order_intents SET status = $2, updated_at = NOW() WHERE id = $1",
    )
    .bind(body.intent_id.as_str())
...
pub async fn list_external_orders(
    ...
    let total_row = sqlx::query(
        "SELECT COUNT(*) AS total FROM external_orders WHERE owner = $1",
    )
    .bind(user.wallet_address.as_str())
    .fetch_one(state.db.pool())
    .await
...
pub async fn update_external_agent(
    ...
    let next_price = body.price.unwrap_or_else(|| current.try_get("price").unwrap_or(0.5));
    ...
    let next_name = body
        ...
        .trim()
        .to_string();
    let next_active = body.active.unwrap_or_else(|| current.try_get("active").unwrap_or(true));
