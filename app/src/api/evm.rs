...
async fn fetch_internal_market_snapshots(state: &AppState) -> Result<Vec<BaseMarketSnapshot>, ApiError> {
...
pub async fn get_base_markets(
    ...
    if matches!(source, ExternalMarketSource::All | ExternalMarketSource::Internal) {
...
pub async fn get_base_market(
    ...
    let market_id = market_id_raw.parse::<u64>().map_err(|_| {
        ApiError::bad_request("INVALID_MARKET_ID", "market_id must be numeric or namespaced")
    })?;
